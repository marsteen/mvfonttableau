...
namespace NUtf
{
    unsigned int Utf8_to_Utf32(const unsigned char* Utf8, int* Size)
    {
        ...
        if ((Utf8[0] & 0xC0) == 0xC0)      // zweistellig, beginnt mit 110
        {
            Utf32 = ShiftBits2(Utf8);
            *Size = 2;
        }
        else
        if ((Utf8[0] & 0xE0) == 0xE0)      // dreistellig, beginnt mit 1110
        {
            Utf32 = ShiftBits3(Utf8);
            *Size = 3;
        }
        else
        if ((Utf8[0] & 0xF0) == 0xF0)      // vierstellig, beginnt mit 11110
        {
    ...
    std::string ToUpper(const std::string in_str)
    {
        ...
        for (std::string::const_iterator it = in_str.begin(); it != in_str.end(); ++it)
        {
            out_str += towupper(*it);
        }
        return out_str;
