...
void CGL_Freetype::Init(const char* Fontfile, int FontSize)
{
  ...
  if (error)
  {
	...
	else
	{
       ...
       cout << "Font height:" << mFace->size->metrics.height << endl;
			 cout << "Font y_ppem:" << mFace->size->metrics.y_ppem << endl;
		}
	}
...
float CGL_Freetype::GetStringHeight() const
{
	return mFace->size->metrics.y_ppem;
}
...
int CGL_Freetype::RenderStringRects(int x, int y, const int WindowWidth, const int WindowHeight, const unsigned char* text, CRectT<float>* rc)
{
	...
	for (int i = 0; text[i] != 0; )
	{
    int yt = WindowHeight - y - GetStringHeight();
		int ThisIndex;
		int cs;
		int Utf32 = NUtf::Utf8_to_Utf32(text + i, &cs);
		float w = RenderGlyph(NULL, x, yt, Utf32, &LastIndex);
      ...
      //cout << "*************************************" << endl;
			int Errors;
			unsigned char utfCharBuff[8];
			memset(utfCharBuff, 0, 8);
			for (int j = 0; j < cs; j++)
			{
				utfCharBuff[j] =  text[i+j];
        //cout << std::hex << (int) utfCharBuff[j] << " ";
			}
			//cout << endl;
			
			std::string AnsiChar = NUtf2::Utf8_to_Ansi(utfCharBuff, &Errors);
			
			if (Errors == 0)
			{
				int index = (unsigned char) AnsiChar[0];
        
        //cout << std::dec << "index=" << index << std::hex << "(" << index << ")" << endl;
        //cout << "*************************************" << endl;
				...
				vrc.top    = y;
				vrc.right  = x + w;
				vrc.bottom = y + GetStringHeight();
				
				
				
				rc[index].left   = float(x) / WindowWidth;
				rc[index].top    = float(y) / WindowHeight;
				rc[index].right  = float(x + w) / WindowWidth;
				rc[index].bottom = float(y + GetStringHeight() * 1.2f) / WindowHeight;
		
				if (index == 228) // ä
				{
					
					cout << "int   left=" << vrc.left  << " top=" << vrc.top << " top=" << vrc.right << " bottom=" << vrc.bottom << endl;				
					cout << "float left=" << rc[index].left  << " top=" << rc[index].top << " right=" << rc[index].right << " bottom=" << rc[index].bottom << endl;				
				}
...
int GetBit(const CDataRect* dr, int x, int y, int pitch)
{
	unsigned char* byte = ((unsigned char*) dr->mData) + (y * pitch) + (x / 8);	
	return *byte & (1 << (7-(x % 8)));
}

static void TwoBitsTo8(CDataRect* dr, int pitch)
{
	int NewWidth = dr->mWidth;
	unsigned char* byteptr = new unsigned char[NewWidth * dr->mHeight];
	memset(byteptr, 0, NewWidth * dr->mHeight);
	//cout << "w=" << dr->mWidth << " h=" << dr->mHeight << endl;
  ...
  for (int y = 0; y < dr->mHeight; y++)
	{
		...
		{
			int offset = y * NewWidth + x;
			if (GetBit(dr, x, y, pitch) > 0)
			{
			  byteptr[offset] = 255;
			}
			else
	...
	{			
		...
		if (glyph_index != 0)
		{
			...
			if (it == mGlyphSet.end())
			{
				...
				if (mRenderModeMonochrome)
				{			
					error = FT_Load_Glyph(mFace,                   /* handle to face object */
																glyph_index,              /* glyph index */
																FT_LOAD_TARGET_MONO | FT_LOAD_MONOCHROME | FT_LOAD_RENDER);         /* load flags, see below */
					...
					dr.mWidth  = mFace->glyph->bitmap.width;
				...
				/*
				cout << "pixel_mode=" << (int) mFace->glyph->bitmap.pixel_mode << endl;
				cout << "width     =" << (int) mFace->glyph->bitmap.width << endl;
				cout << "pitch     =" << (int) mFace->glyph->bitmap.pitch << endl;
				*/
	...
	{		
		{
			...
			*LastIndex = GlyphTex->mGlyphIndex;
			...
			NGLprimitives::DrawRect(&vr, &GlyphTex->mTexRect);
...
float CGL_Freetype::RenderGlyph(const CGL_Context* glcon, float x,  float y, const int c, int* LastIndex, CRectT<float>* vrOut)
{
	float Kerning = 0;
	int error;
	int Advance;
